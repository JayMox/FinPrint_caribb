...
-EFFORT aggregated on habitat codes, not sites
...
paddack <- read_csv(here('data', 'lkup_trophic_paddack.csv')) %>% 
  janitor::clean_names() %>% 
  mutate(sci.name = paste(genus, species, sep = " ")) %>% 
  select(sci.name, trophic_group)

for(j in 1:nrow(dB)){
  print(paste("THIS IS ", first(data.frame(dB$fish[j])$country)))
  print((colnames(data.frame(dB$fish[j]))))
}

dB <- dB %>% mutate(sc = fish %>% merge(paddack %>% select(sci.name, ), by = 'sci.name', all.x = T))
```
